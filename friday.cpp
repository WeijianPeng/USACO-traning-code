...
int main()
{
	freopen("friday.txt","r",stdin);
	freopen("friday.out","w",stdout);

	int year=1900,a_day=0,week[7],month[12]={31,28,31,30,31,30,31,31,30,31,30,31};
	int n,nleap=0,i,j;
	...
	for (i=0;i<n;i++){
		for (j=0;j<12;j++){
			...
			if(j==1&&((year%4==0&&year%100!=0)||year%400==0))
				a_day=(a_day+month[j]+1)%7;
			else
				a_day=(a_day+month[j])%7;
		}
	}
